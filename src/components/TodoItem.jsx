    ...
    return (
        <li className={'todo-list__item'}>
            <button onClick={() => completeTodo(todo.id)}>
                <FiCheck size={30} className={'todo-item__complete-btn'} />
            </button>
            ...
            {editId === todo.id ? (
                <button className={'todo-item__save-btn'} onClick={() => handleSave(todo.id, todo.task)}>
                    Сохранить
                </button>
            ) : (
            ...
            <button onClick={() => removeTodo(todo.id)}>
                <FiTrash2 size={30} className={'todo-item__remove-btn'} />
            </button>
        </li>
    );
};
